...
function createSignature(){
    ...
    var cert = fs.readFileSync('webPush/website_aps_production.pem'),
        key = fs.readFileSync('webPush/webpushcertificate.pem'),
        websiteJson = pushLib.websiteJSON(
            "Safari Push Notification Test", // websiteName 
            "web.com.iomedia.safaripushtest", // websitePushID 
            ["http://localhost:5000"], // allowedDomains 
            "http://localhost:5000/%@/", // urlFormatString 
            0123456789012345, // authenticationToken (zeroFilled to fit 16 chars) 
            "http://localhost:5000" // webServiceURL (Must be https!) 
        );
